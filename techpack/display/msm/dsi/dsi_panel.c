				...
				enum dsi_cmd_set_type type)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (type == DSI_CMD_SET_MI_DC_ON || type == DSI_CMD_SET_MI_DC_OFF)
			cmds->msg.flags |= MIPI_DSI_MSG_CMD_DMA_SCHED;
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (is_backlight_set_skip(panel)) {
		mi_dsi_panel_update_last_bl_level(panel, bl_lvl);
		return 0;
	} else if (panel->mi_cfg.aod_bl_51_ctl && panel->power_mode == SDE_MODE_DPMS_LP1) {
		DSI_INFO("%s panel skip set backlight %d due to display is lp1 state\n", panel->type, bl_lvl);
...
const char *cmd_set_prop_map[DSI_CMD_SET_MAX] = {
	...
	"mi,mdss-dsi-set-cup-dbi-command",
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	"mi,mdss-dsi-nolp-dc-lbm-command-state",
	...
	"mi,mdss-dsi-set-cup-dbi-command-state",
...
int dsi_panel_set_lp1(struct dsi_panel *panel)
{
	...
	if (panel->mi_cfg.aod_bl_51_ctl && panel->power_mode == SDE_MODE_DPMS_LP2) {
...
exit:
	...
	display = mi_get_dsi_display(MI_DISP_PRIMARY);
...
int dsi_panel_set_nolp(struct dsi_panel *panel)
{
	...
	if (panel->mi_cfg.panel_state == PANEL_STATE_ON && !panel->mi_cfg.aod_bl_51_ctl) {
